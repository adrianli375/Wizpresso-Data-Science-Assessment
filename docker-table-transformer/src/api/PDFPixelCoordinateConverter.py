...
def convert_pixel_to_pdf_coordinates(image, start_x, end_x, start_y, end_y, border_size, conversion_ratio=2):
    ...
    """
    width, height = image.size
    adjusted_start_x = (start_x - border_size) / conversion_ratio
    adjusted_end_x = (end_x - border_size) / conversion_ratio
    adjusted_start_y = ((height - border_size*2) - (end_y - border_size)) / conversion_ratio
    adjusted_end_y = ((height - border_size*2) - (start_y - border_size)) / conversion_ratio
    return floor(adjusted_start_x), ceil(adjusted_end_x), floor(adjusted_start_y), ceil(adjusted_end_y)
